...
def profile(user:User=Depends(current_user),db:Session=Depends(get_db)):
 rows=db.scalars(select(Progress).where(Progress.user_id==user.id).order_by(Progress.last_played.desc())).all();return {'user':user.__dict__|{},'progress':[{'incident_id':r.incident_id,'solved':r.solved,'best_score':r.best_score,'attempts':r.attempts,'last_played':r.last_played} for r in rows]}
...
def inventory(user:User=Depends(current_user),db:Session=Depends(get_db)):
 return [{'id':x.id,'item_key':x.item_key,'name':x.name,'icon':x.icon,'quantity':x.quantity,'description':x.description} for x in db.scalars(select(InventoryItem).where(InventoryItem.user_id==user.id)).all()]
@router.get('/companions')
def companions(user:User=Depends(current_user),db:Session=Depends(get_db)):
 return [{'id':x.id,'name':x.name,'role':x.role,'emoji':x.emoji,'description':x.description,'hp':x.hp} for x in db.scalars(select(Companion).where(Companion.user_id==user.id)).all()]
@router.get('/chat')
def chat(user:User=Depends(current_user),db:Session=Depends(get_db)):
 return [{'id':x.id,'role':x.role,'content':x.content,'created_at':x.created_at} for x in db.scalars(select(ChatMessage).where(ChatMessage.user_id==user.id).order_by(ChatMessage.created_at.asc()).limit(100)).all()]
@router.post('/chat')
def send_chat(data:ChatIn,user:User=Depends(current_user),db:Session=Depends(get_db)):
 msg=ChatMessage(user_id=user.id,role='user',content=data.content.strip());db.add(msg);db.commit();db.refresh(msg)
 # AI provider is intentionally isolated here. Replace this stub with OpenAI/other provider later.
...
async def avatar(file:UploadFile=File(...),user:User=Depends(current_user),db:Session=Depends(get_db)):
 ...
 root=Path(settings.upload_dir)/'avatars';root.mkdir(parents=True,exist_ok=True);name=f'{user.id}-{uuid4().hex}{allowed[file.content_type]}';path=root/name;path.write_bytes(data)
 user.avatar_url=f'/uploads/avatars/{name}';db.commit();return {'avatar_url':user.avatar_url}
