...
def dnd_roll(data: RollIn, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if data.kind=='save':
        ...
        if not ability: raise HTTPException(404,'Характеристика для спасброска не найдена')
        p=prof if ability in derived['saving_throw_proficiencies'] else 0
        return {'kind':'save','key':ability,'name':ABILITIES[ability],**d20_roll(ability_modifier(stats[ability]),data.target,p,data.advantage)}
    if data.kind=='attack':
